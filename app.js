...
async function loginUser(chapa, nombre = null) {
  ...
  // Pedir correo si no existe (una vez por sesion)
  checkAndPromptEmail();
...
function setVideoPlaybackRate() {
  ...
  const playbackRate = prefersReducedMotion ? 1 : 1.5;
  videos.forEach((video) => {
    video.preload = 'none'; // Lazy loading: no precargar videos
...
let openEmailCaptureModal = null;
...
function initEmailCaptureModal() {
  ...
  const saveBtn = document.getElementById('save-email-capture');

  const chapaInput = document.getElementById('email-capture-chapa');
  const emailInput = document.getElementById('email-capture-input');
  const errorMsg = document.getElementById('email-capture-error');
  const successMsg = document.getElementById('email-capture-success');
  ...
  // Modal obligatorio: sin cierre por overlay ni por boton
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (error) {
        ...
        showError('No se pudo guardar el correo. Intentalo de nuevo.');
        saveBtn.disabled = false;
        saveBtn.textContent = 'Guardar correo';
        return;
      }

      showSuccess('Correo guardado correctamente.');
      setTimeout(() => {
        modal.style.display = 'none';
        resetState();
      }, 1200);
...
async function checkAndPromptEmail() {
  if (!AppState.currentUser || typeof supabase === 'undefined') return;
  if (typeof openEmailCaptureModal !== 'function') return;
  ...
  try {
    ...
    if (error) {
      console.warn('⚠️ No se pudo verificar email del usuario:', error);
      return;
    }

    if (!data?.email || data.email.trim() === '') {
      openEmailCaptureModal(AppState.currentUser);
    }
  } catch (error) {
    console.warn('⚠️ Error verificando email del usuario:', error);
  }
}
